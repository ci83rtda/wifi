...
{{--            <a href="{{ route('registration') }}" class="font-medium text-indigo-600 hover:text-indigo-500">pulsa aquí para comenzar</a>.--}}
{{--        </p>--}}
    </div>
    ...
    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div class="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
            <form class="space-y-6" action="{{ route('access.verify') }}" method="POST">
                @csrf
                @method('post')
                ...
                <div>
                    <button type="submit" class="flex w-full justify-center rounded-md border border-transparent bg-indigo-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Continuar</button>
                </div>
            </form>
